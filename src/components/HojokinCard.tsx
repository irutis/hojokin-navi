...
export default function HojokinCard({ h }: { h: Hojokin }) {
  const isUrgent = h.deadline !== '随時受付' && !h.deadline.includes('継続')

  return (
    <Link href={`/hojokin/${h.slug}`} className="block group">
      <div className="border border-gray-200 rounded-xl p-5 bg-white hover:border-blue-400 hover:shadow-md transition-all h-full flex flex-col">
        ...
        <p className="text-sm text-gray-500 mb-4 line-clamp-2 flex-1">{h.purpose}</p>
        <div className="flex items-end justify-between">
          ...
          <span className="text-xs text-gray-400 shrink-0">{h.managing_org.replace('経済産業省・', '').replace('厚生労働省', '厚労省')}</span>
        </div>
      </div>
    </Link>
  )
